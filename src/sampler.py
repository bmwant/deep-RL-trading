...
class PairSampler(Sampler):
    def __init__(
        ...
        param_str = str(
            (
                self.noise_level,
                self.forecast_horizon_range,
                self.n_section,
                self.windows_transform,
            )
        )
    ...
    def __randwalk(self, l):
        change = (
            (np.random.random(l + self.forecast_horizon_range[1]) - 0.5)
            * 2
            * self.max_change_perc
            / 100
        )
        forecast_horizon = random.randrange(
            self.forecast_horizon_range[0], self.forecast_horizon_range[1]
        )
        return (
            change[:l],
            change[forecast_horizon : forecast_horizon + l],
            forecast_horizon,
        )

    def __randjump(self, l):
        change = [0.0] * (l + self.forecast_horizon_range[1])
        n_jump = random.randrange(15, 30)
        for i in range(n_jump):
            t = random.randrange(len(change))
            change[t] = (
                (np.random.random() - 0.5) * 2 * self.max_change_perc / 100
            )
        forecast_horizon = random.randrange(
            self.forecast_horizon_range[0], self.forecast_horizon_range[1]
        )
    ...
    def __sample(self):
        ...
        price = 100.0 * (1.0 + np.cumsum(d_price))
        signal = (
            100.0 * (1.0 + np.cumsum(d_signal))
...
class SinSampler(Sampler):
    ...
    def __rand_sin(
        ...
        p = 100.0 + amplitude * np.sin(
            np.array(range(length)) * 2 * 3.1416 / period
        )
...
def test_PairSampler():
    ...
    fld = os.path.join(
        ...
        "PairSamplerDB",
        game
        + "_%i,%i" % (n_episodes, n_section)
        + str(fhr)
        + str(windows_transform)
        + "_B",
    )
    sampler.build_db(n_episodes, fld)
    # """
