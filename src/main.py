...
def get_model(model_type, env, learning_rate, fld_load):
    ...
    if model_type == "MLP":
        ...
        model.build_model(
            hidden_size, learning_rate=learning_rate, activation="tanh"
        )
        ...
        model.build_model(
            hidden_size, dense_units, learning_rate=learning_rate
        )
        print_t = True
...
def main():
    ...
    # print('='*20+'\nin-sample testing\n'+'='*20)
    simulator.test(
        n_episode_testing, save_per_episode=1, subfld="in-sample testing"
    )
