...
class Agent:
    ...
    def replay(self):
        ...
        for (
            ...
            q = reward
            if not done:
                q += self.discount_factor * np.nanmax(
                    self.get_q_valid(next_state, next_valid_actions)
                )
...
class QModelRNN(QModelKeras):
    ...
    def _build_model(
        self, Layer, n_hidden, dense_units, learning_rate, activation="relu"
    ):

        model = keras.models.Sequential()
        model.add(
            keras.layers.Reshape(
                self.state_shape, input_shape=self.state_shape
            )
        )
        m = len(n_hidden)
        for i in range(m):
            model.add(Layer(n_hidden[i], return_sequences=(i < m - 1)))
        for i in range(len(dense_units)):
            model.add(
                keras.layers.Dense(dense_units[i], activation=activation)
            )
        model.add(keras.layers.Dense(self.n_action, activation="linear"))
        model.compile(
            loss="mse", optimizer=keras.optimizers.Adam(lr=learning_rate)
...
class QModelLSTM(QModelRNN):
    ...
    def build_model(
        self, n_hidden, dense_units, learning_rate, activation="relu"
    ):
        Layer = keras.layers.LSTM
        self._build_model(
            Layer, n_hidden, dense_units, learning_rate, activation
        )
...
class QModelConv(QModelKeras):
    ...
    def build_model(
        ...
        self.model_name = (
            self.qmodel
            + str(
                [a for a in zip(filter_num, filter_size, dilation, use_pool)]
            )
            + " + "
            + str(dense_units)
        )
...
class QModelConvRNN(QModelKeras):
    ...
    def _build_model(
        ...
        m = len(RNN_n_hidden)
        for i in range(m):
            model.add(RNNLayer(RNN_n_hidden[i], return_sequences=(i < m - 1)))
        for i in range(len(dense_units)):
            model.add(
                keras.layers.Dense(dense_units[i], activation=activation)
            )
