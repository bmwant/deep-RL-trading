...
class Visualizer:
    ...
    def plot_episodes(
        ...
        if explored_total_rewards is None:
            ...
            ax_exploration = plt.subplot2grid(
                figshape, (2, 0), sharex=ax_reward
            )
        ...
        if explored_total_rewards is not None:
            ax_exploration.plot(tt, np.array(explorations) * 100.0, "k")
    ...
    def __init__(self, model):
        ...
        for layer in self.layers:
            self.inter_models[layer] = keras.models.Model(
                inputs=model_input, outputs=self.model.get_layer(layer).output
            )
        self.config()
